...
if (message === "hi" || message === "menu") {
  ...
  try {
    ...
    if (menuRes && menuRes.rows && menuRes.rows.length) {
      imageUrl = menuRes.rows[0].menu_image_url;
    }
...
`);
    ...
    const textMenu = buildMenuText(rows);

    return res.send(`<Response><Message>${textMenu}</Message></Response>`);
    ...
    return res.send(`
      <Response>
        <Message>Failed to load menu. Try again.</Message>
      </Response>
    `);
  }
}
...
//     return res.send(`
//       <Response>
//         <Message>Backend working ✅</Message>
//       </Response>
//     `);
